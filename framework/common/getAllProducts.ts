...
export type ApiFetcherOptions = {
  ...
  export const getAllProducts = async (config: ApiConfig): Promise<ProductTest[]> => {
    const { data } = await config.fetch<ReturnType>({
      query: getAllProductsQuery
    })
    console.log(data.products.edges[0].node.variants.edges[0].node)
  ...
  export function normalizeProduct(productNode: ShopifyProduct): ProductTest {
    const {
      id,
      title: name,
      handle,
...
return edges.map(({node}) => {
  ...
  export interface ProductTest {
    ...
    images: ProductImage[]
    price: ProductPrice
    options: ProductOptionTest[],
    variants: ProductVariantTest[]
  }
  export interface ProductOptionTest {
    id: string
    displayName: string
